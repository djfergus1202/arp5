...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("🔬 Meta-Analysis Verification", use_container_width=True):
            st.switch_page("pages/2_Meta_Analysis_Verification.py")
            
        if st.button("✍️ Paper Rewriter", use_container_width=True):
        ...
        if st.button("🧬 Molecular Docking", use_container_width=True):
            st.switch_page("pages/5_Molecular_Docking.py")
            
        if st.button("📋 Validation Results", use_container_width=True):
            st.switch_page("pages/6_Validation_Results.py")
            
        if st.button("🧬 Pharmacological Maps", use_container_width=True, type="primary"):
            st.switch_page("pages/7_Pharmacological_Topological_Maps.py")
            
        if st.button("📚 Data Citations", use_container_width=True):
            st.switch_page("pages/8_Data_Citations.py")
            
        if st.button("📈 Plot Implications", use_container_width=True):
            st.switch_page("pages/9_Plot_Implications.py")
        
        st.markdown("---")
        st.markdown("**Note**: Use the navigation buttons above to access different research modules.")
