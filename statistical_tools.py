...
StatisticalAnalyzer = StatisticalValidator
    ...
    def leave_one_out_analysis(self, effect_sizes: List[float], 
                              standard_errors: List[float]) -> List[Dict[str, Any]]:
        """Perform leave-one-out sensitivity analysis"""
        
        if len(effect_sizes) < 3:
            raise ValueError("Need at least 3 studies for leave-one-out analysis")
        
        results = []
        
        for i in range(len(effect_sizes)):
            # Remove study i
            loo_es = effect_sizes[:i] + effect_sizes[i+1:]
            loo_se = standard_errors[:i] + standard_errors[i+1:]
            
            # Perform meta-analysis without study i
            loo_result = self.random_effects_meta_analysis(loo_es, loo_se)
            
            results.append({
                'excluded_study': i,
                'pooled_effect': loo_result.pooled_effect,
                'confidence_interval': loo_result.confidence_interval,
                'i_squared': loo_result.i_squared,
                'p_value': loo_result.p_value
            })
        
        return results
    
    def calculate_sample_size_meta_analysis(self, expected_effect: float, 
                                          power: float = 0.8,
                                          alpha: float = 0.05,
                                          heterogeneity: float = 0.0) -> Dict[str, int]:
        """Calculate required number of studies for meta-analysis"""
        ...
        # This is simplified - real calculation would depend on study designs
        
        variance_total = 1 + heterogeneity  # Simplified variance including heterogeneity
        
        n_studies = ((z_alpha + z_beta)**2 * variance_total) / (expected_effect**2)
        
        return {
            'required_studies': math.ceil(n_studies),
            'power': power,
            'alpha': alpha,
            'expected_effect': expected_effect,
            'assumed_heterogeneity': heterogeneity
        }
    
    def validate_effect_sizes(self, effect_sizes: List[float], 
                             standard_errors: List[float],
                             study_names: Optional[List[str]] = None) -> Dict[str, Any]:
        """Validate effect sizes and identify potential issues"""
        ...
        if len(effect_sizes) != len(standard_errors):
            validation_results['issues'].append("Mismatched effect sizes and standard errors")
            return validation_results
        
        for i, (es, se) in enumerate(zip(effect_sizes, standard_errors)):
            study_name = study_names[i] if study_names else f"Study {i+1}"
            
            # Check for valid values
            if math.isnan(es) or math.isnan(se):
                validation_results['issues'].append(f"{study_name}: Contains NaN values")
                continue
            
            if math.isinf(es) or math.isinf(se):
                validation_results['issues'].append(f"{study_name}: Contains infinite values")
                continue
            
            if se <= 0:
                validation_results['issues'].append(f"{study_name}: Non-positive standard error")
                continue
            
            # Check for extreme values
            if abs(es) > 10:
                validation_results['extreme_values'].append(f"{study_name}: Very large effect size ({es:.2f})")
            
            if se > 5:
                validation_results['extreme_values'].append(f"{study_name}: Very large standard error ({se:.2f})")
            
            validation_results['valid_studies'] += 1
        
        # Check for outliers using standardized residuals
        if validation_results['valid_studies'] >= 3:
            try:
                ma_result = self.random_effects_meta_analysis(effect_sizes, standard_errors)
                
                for i, (es, se) in enumerate(zip(effect_sizes, standard_errors)):
                    if math.isfinite(es) and math.isfinite(se) and se > 0:
                        study_name = study_names[i] if study_names else f"Study {i+1}"
                        
                        # Standardized residual
                        residual = (es - ma_result.pooled_effect) / se
                        
                        if abs(residual) > 2.5:  # Outlier threshold
                            validation_results['outliers'].append(f"{study_name}: Potential outlier (residual: {residual:.2f})")
            
            except Exception as e:
                validation_results['issues'].append(f"Could not perform outlier analysis: {str(e)}")
